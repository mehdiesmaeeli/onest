...
var BOT_TOKEN = "104666245:AAFeGCE2OaxaPcWb3A4Nq0oKjE4OCdOwWu0";
...
var getstart = function (message) {
    ...
    var answer = {
        ...
        text: 'سلام ' + message.chat.first_name + 'عزیز ، به ربات فروشگاه وانست خوش آمدید ' + '\n' +
            'با این ربات شما می توانید محصولات جدید فروشگاه وانست را مشاهده کنید' + '\n' +
            'شما می توانید از دستورات زیر در این ربات استفاده کنید: \n /start - آشنایی با ربات \n /help - دستورات ربات  \n /order - سفارشات ویژه \n /contact - تماس با فروشگاه'
        ,
        reply_markup: JSON.stringify({
            keyboard: [["/start", "/help" , "/order" , "/contact" ]],
...
var nocommand = function (message) {
    // Fetch a question from database.
    // We do this by randomly getting an array entry --> in a real world app you would query the database for questions the current user hasn't answered yet
    //var question = questions[randomInt(questions.length - 1)];
    ...
    var answer = {
        chat_id: message.chat.id,
        text: 'سلام '+ message.chat.first_name  + 'عزیز ، شما از دستور اشتباهی استفاده کرده اید برای دریافت لیست دستورات از  \n /help \n استفاده کنید.  '
        ,
        reply_markup: JSON.stringify({
            keyboard: [["/help"]],
            resize_keyboard: true,
...
var gethelp = function (message) {
    ...
    var answer = {
        chat_id: message.chat.id,
        text: 'شما می توانید از دستورات زیر استفاده کنید: \n /start - آشنایی با ربات \n /help - دستورات ربات  \n /order - سفارشات ویژه \n /contact - تماس با فروشگاه'
        ,
        reply_markup: JSON.stringify({
            keyboard: [["/start", "/help", "/order", "/contact"]],
            resize_keyboard: true,
...
var getcontact = function (message) {
    // Fetch a question from database.
    // We do this by randomly getting an array entry --> in a real world app you would query the database for questions the current user hasn't answered yet
    //var question = questions[randomInt(questions.length - 1)];
    ...
    var answer = {
        chat_id: message.chat.id,
        text: 'شما می توانید از طریق سایت و یا تلگرام با ما در ارتباط باشید : \n site: Onest.ir \n telegram:telegram.me/onestgram \n '
    };
...
var getquestion = function(message) {
    ...
    // Build the telegram answer message object with keyboard buttons
    var list = [{ name: 'AgADBAAD46kxG4NXxwPFJY88QURhEARsYzAABFH67sqEV2B_-RABAAEC', caption: 'escada \n 140T' }, { name: 'AgADBAAD5KkxG4NXxwNoY6VMuVLxZYtyYzAABPasEgQ0GrISqxYBAAEC', caption: 'barberi \n 160T' }, { name: 'AgADBAAD5akxG4NXxwN1kApCe_Zklr7aajAABJEfNOZIUodCnFAAAgI', caption: 'bag \n 128T' }]
    for (var i = 0; i < 3; i++) {
        var answer = {
            chat_id: message.chat.id,
            photo:list[i].name ,
...
function runCommand(message) {
	...
	if (msgtext.indexOf("/") != 0) {
	    nocommand(message);
	    return false;
	}// no slash at beginning? --> no command --> return
